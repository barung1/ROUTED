...
const Sidebar: React.FC = () => {
  ...
  const Item: React.FC<{to: string; children: React.ReactNode}> = ({ to, children }) => (
    <Link to={to} className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm ${pathname === to ? 'bg-brand-50 text-brand-700 font-medium' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}>
      {children}
    </Link>
  )
  ...
  return (
    <aside className="hidden md:flex md:flex-col md:w-64 md:h-screen md:sticky md:top-0 bg-white border-r border-gray-100 p-4">
      <div className="mb-6 flex items-center gap-3">
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-brand-500 to-brand-600 shadow" />
